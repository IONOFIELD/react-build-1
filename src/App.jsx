...
const controlBtn = (active = false) => ({
  padding: "4px 10px", background: active ? "#1a2e1a" : "#111",
  border: `1px solid ${active ? "#22c55e40" : "#222"}`, borderRadius: 3,
  color: active ? "#4ade80" : "#888", fontSize: 11, cursor: "pointer",
...
function StatusBadge({ status }) {
  const cfg = {
    verified: { icon: I.Check(), bg: "#052e16", border: "#166534", text: "#4ade80", label: "Verified" },
    ...
    <span style={{display:"inline-flex",alignItems:"center",gap:5}}>
      {record.isTest && (
        <span title="Test file" style={{
          ...
          cursor:"help",boxShadow:"0 0 5px #4ade8060",
        }}/>
      )}
      <span style={{display:"inline-flex",alignItems:"center",padding:"2px 8px",borderRadius:3,fontSize:10,fontWeight:700,
  ...
  return (
    <div ref={wrapRef} style={{position:"relative",zIndex:open?30:1}}>
      <button onClick={(e)=>{e.stopPropagation();setOpen(!open);setConfirmDelete(false);}} style={{
        ...
        borderRadius:3,cursor:"pointer",color:open?"#ccc":"#555",transition:"all 0.15s",
      ...
      {open && (
        <div style={{
          ...
          width:200,background:"#111",border:"1px solid #2a2a2a",borderRadius:6,
            ...
            <div style={{padding:12}}>
              ...
              <div style={{display:"flex",gap:6}}>
                <button onClick={(e)=>{e.stopPropagation();setConfirmDelete(false);setOpen(false);}} style={{
                  flex:1,padding:"5px 0",background:"#111",border:"1px solid #333",borderRadius:3,
                ...
                <button onClick={(e)=>{e.stopPropagation();tauriBridge.deleteFiles(record.studyType,record.filename);onDelete(record.id);setOpen(false);setConfirmDelete(false);}} style={{
                  flex:1,padding:"5px 0",background:"#7f1d1d",border:"1px solid #EF444440",borderRadius:3,
...
function StatusControl({ status, onSetStatus, size = "normal" }) {
  const statuses = [
    ...
    { key: "verified", icon: I.Check(),  color: "#4ade80", border: "#166534", bg: "#052e16", label: "Verified" },
...
function WaveformCanvas({ channels, waveformData, epochSec, epochStart, epochEnd, sampleRate,
  ...
  const drawEEG = useCallback(() => {
    ...
    channels.forEach((ch, i) => {
      ...
      ctx.strokeStyle = ch === "EKG" ? "#EC489980" : "#4ade8080";
    ...
    <div style={{ width:260,borderLeft:"1px solid #1a1a1a",background:"#0c0c0c",
      ...
      <div style={{padding:"10px 12px",borderTop:"1px solid #1a1a1a"}}>
        <button onClick={()=>{
          ...
          borderRadius:4,color:"#888",cursor:"pointer",fontSize:11,fontWeight:600,
    ...
    <div style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:12,
      ...
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:11,color:"#888"}}>
          Epoch <span style={{color:"#4ade80",fontWeight:700}}>{currentEpoch+1}</span>
      ...
      <input type="range" min={0} max={totalEpochs-1} value={currentEpoch}
        onChange={e=>setCurrentEpoch(parseInt(e.target.value))} style={{width:200,accentColor:"#4ade80"}}/>
...
function AnnotationPopup({ draft, annotationType, text, setText, onConfirm, onCancel, containerRef }) {
  ...
  return (
    <div style={{
      ...
      background:"#111", border:`1px solid ${ac.color}40`, borderRadius:6,
      padding:"10px 14px", display:"flex", alignItems:"center", gap:8,
      zIndex:10, boxShadow:`0 4px 24px rgba(0,0,0,0.6), 0 0 0 1px ${ac.color}15`,
      whiteSpace:"nowrap",
    }}>
      ...
      <input value={text} onChange={e=>setText(e.target.value)} placeholder="Add note..."
        style={{ background:"#0a0a0a",border:"1px solid #222",borderRadius:3,
          color:"#e0e0e0",fontSize:11,padding:"4px 8px",width:160,outline:"none" }}
        autoFocus onKeyDown={e=>e.key==="Enter"&&onConfirm()}/>
      <button onClick={onConfirm} style={{
        padding:"4px 10px",background:"#166534",border:"1px solid #22c55e40",
        borderRadius:3,color:"#4ade80",fontSize:10,fontWeight:700,cursor:"pointer"
      }}>SAVE</button>
...
function ChannelContextMenu({ x, y, channelName, isHidden, channelSens, onToggleVisibility, onAdjustSensitivity, onClose }) {
  ...
  return (
    <div ref={menuRef} style={{
      ...
      <div style={{padding:"6px 12px",borderBottom:"1px solid #1a1a1a",fontSize:10,color:"#4ade80",fontWeight:700}}>
        {channelName}
      </div>
      {item(isHidden ? "Show Channel" : "Hide Channel", isHidden ? "#4ade80" : "#888", () => { onToggleVisibility(); onClose(); })}
      ...
      <div style={{padding:"6px 12px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:4}}>
          <button onClick={()=>onAdjustSensitivity(-1)} style={{
            width:22,height:22,background:"#0a0a0a",border:"1px solid #333",borderRadius:3,
...
function LibraryTab({ records, setRecords, onOpenReview, updateRecordStatus }) {
  ...
  const inputStyle = {
    ...
    borderRadius:4,color:"#e0e0e0",fontSize:13,fontFamily:"'IBM Plex Mono', monospace",outline:"none",boxSizing:"border-box",
  };
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden"}}>
      ...
      <div style={{padding:"14px 28px",display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid #1a1a1a",flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,background:"#0d0d0d",border:"1px solid #2a2a2a",borderRadius:4,padding:"0 10px",flex:"1 1 200px"}}>
          {I.Search()}
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          <select value={filterType} onChange={e=>setFilterType(e.target.value)}
            style={{background:"#0d0d0d",border:"1px solid #2a2a2a",borderRadius:4,color:"#aaa",fontSize:12,padding:"6px 8px",outline:"none"}}>
        ...
        <div style={{display:"flex",background:"#0d0d0d",border:"1px solid #2a2a2a",borderRadius:4,overflow:"hidden"}}>
          {["table","grid"].map(m=>(
        ...
        <button onClick={()=>setShowIngest(true)} style={{
          padding:"8px 16px",background:"#166534",border:"1px solid #22c55e50",borderRadius:6,
          color:"#4ade80",cursor:"pointer",fontSize:12,fontWeight:700,display:"flex",alignItems:"center",gap:6
        }}>{I.Plus()} INGEST</button>
        <button onClick={()=>setShowExport(true)} style={{
          padding:"8px 16px",background:"#111",border:"1px solid #3B82F640",borderRadius:6,
        ...
        {viewMode==="table" ? (
                  ...
                  <td style={{padding:"10px 12px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:4}}>
                      <button onClick={()=>onOpenReview(r)} style={{
                        padding:"4px 10px",background:"#111",border:"1px solid #222",borderRadius:3,
                        color:"#4ade80",fontSize:10,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:4
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:12,padding:"20px 28px"}}>
            {filtered.map(r=>{
              const st=STUDY_TYPES[r.studyType]||{label:"?",color:"#666"};
              return (
                <div key={r.id} style={{background:"#0d0d0d",border:"1px solid #1a1a1a",borderRadius:6,padding:16,cursor:"pointer",transition:"border-color 0.15s"}}
                  ...
                  <div style={{fontFamily:"'IBM Plex Mono', monospace",fontSize:11,color:"#4ade80",marginBottom:10,wordBreak:"break-all"}}>{r.filename}</div>
      ...
      {/* Ingest Modal */}
      {showIngest && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,backdropFilter:"blur(4px)"}} onClick={()=>setShowIngest(false)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#111",border:"1px solid #2a2a2a",borderRadius:8,padding:28,width:480,maxHeight:"80vh",overflow:"auto"}}>
      ...
      {/* Export Modal */}
      {showExport && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,backdropFilter:"blur(4px)"}} onClick={()=>setShowExport(false)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#111",border:"1px solid #2a2a2a",borderRadius:8,padding:28,width:520,maxHeight:"80vh",overflow:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
              <h3 style={{margin:0,color:"#e0e0e0",fontSize:16,fontWeight:700}}>Export Patient Data</h3>
              <button onClick={()=>setShowExport(false)} style={{background:"none",border:"none",color:"#666",cursor:"pointer",padding:4}}>{I.X()}</button>
            </div>
            <div style={{fontSize:11,color:"#555",marginBottom:16}}>
              Select a subject to export all their EDF files and annotations as a bundled package.
            </div>
            {(() => {
              const subjects = {};
              records.forEach(r => {
              ...
              return Object.values(subjects).map(subj => (
                <div key={subj.hash} style={{
                  background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:6,padding:14,marginBottom:8,
                  display:"flex",alignItems:"center",justifyContent:"space-between",
                }}>
                  <div>
                    <div style={{fontFamily:"'IBM Plex Mono', monospace",fontSize:13,color:"#4ade80",fontWeight:700}}>{subj.hash}</div>
                    <div style={{fontSize:10,color:"#555",marginTop:2}}>
                      {subj.records.length} recording{subj.records.length!==1?"s":""} — {subj.sport}
                    </div>
                    <div style={{fontSize:9,color:"#444",marginTop:4,display:"flex",gap:6,flexWrap:"wrap"}}>
                      {subj.records.map(r => {
                        const st = STUDY_TYPES[r.studyType] || {label:"?",color:"#666"};
                        return <span key={r.id} style={{color:st.color}}>{st.label}</span>;
                      })}
                    </div>
                  </div>
                  <button onClick={()=>{
                    // Bundle all records for this subject into a JSON manifest
                    const manifest = {
                      exportDate: new Date().toISOString(),
                      subjectHash: subj.hash,
                      recordCount: subj.records.length,
                      records: subj.records.map(r => ({
                        filename: r.filename,
                    ...
                    a.href = url;
                    a.download = `REACT-EXPORT-${subj.hash}-${new Date().toISOString().split("T")[0]}.json`;
                    a.click();
                    URL.revokeObjectURL(url);
                    // In Tauri, this would call a Rust command to zip the actual EDF + JSON files
                    if (!window.__TAURI__) {
                      alert(`Export manifest downloaded.\n\nIn the desktop app, this will create a ZIP containing:\n${subj.records.map(r=>`  • ${r.filename}\n  • ${r.filename.replace('.edf','_annotations.json')}`).join('\n')}`);
                    }
                  }} style={{
                    padding:"8px 16px",background:"#0a0a2a",border:"1px solid #3B82F640",borderRadius:4,
                    color:"#3B82F6",cursor:"pointer",fontSize:11,fontWeight:700,display:"flex",alignItems:"center",gap:4,flexShrink:0,
                  }}>{I.Package()} Export</button>
                </div>
              ));
            })()}
      ...
      {selectedRecord && (
        <div style={{position:"fixed",right:0,top:0,bottom:0,width:400,background:"#0d0d0d",borderLeft:"1px solid #2a2a2a",zIndex:999,overflow:"auto",padding:24}}>
          ...
          <div style={{background:"#111",border:"1px solid #2a2a2a",borderRadius:6,padding:16,fontFamily:"'IBM Plex Mono', monospace",fontSize:13,color:"#4ade80",wordBreak:"break-all",marginBottom:20}}>
          ...
          <button onClick={()=>{onOpenReview(selectedRecord);setSelectedRecord(null);}} style={{
            width:"100%",padding:"10px 0",background:"#166534",border:"1px solid #22c55e50",borderRadius:4,color:"#4ade80",
...
function IngestForm({ onClose, onIngest }) {
  ...
  const inputStyle = {width:"100%",padding:"8px 10px",background:"#0d0d0d",border:"1px solid #2a2a2a",borderRadius:4,color:"#e0e0e0",fontSize:13,fontFamily:"'IBM Plex Mono', monospace",outline:"none",boxSizing:"border-box"};
  const formLabel = {display:"block",fontSize:11,color:"#777",marginBottom:4,fontWeight:600,letterSpacing:"0.05em",textTransform:"uppercase"};
  const handleSubmit = () => {
  ...
  return (<>
    ...
    {form.subjectId&&<div style={{background:"#0a0a0a",border:"1px solid #1a3a1a",borderRadius:4,padding:"8px 12px",marginBottom:20,fontFamily:"'IBM Plex Mono', monospace",fontSize:12,color:"#4ade80"}}>
    ...
    <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
      <button onClick={onClose} style={{padding:"8px 16px",background:"transparent",border:"1px solid #333",borderRadius:4,color:"#888",cursor:"pointer",fontSize:13}}>Cancel</button>
      <button onClick={handleSubmit} disabled={!form.subjectId} style={{padding:"8px 20px",background:form.subjectId?"#166534":"#1a1a1a",border:"1px solid "+(form.subjectId?"#22c55e":"#333"),borderRadius:4,color:form.subjectId?"#4ade80":"#555",cursor:form.subjectId?"pointer":"default",fontSize:13,fontWeight:600}}>
    ...
    <div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 16px",borderBottom:"1px solid #1a1a1a",background:"#0a0a0a",fontSize:10,color:"#555"}}>
        <span onClick={()=>setShowFilePicker(!showFilePicker)} style={{
          color:"#4ade80",fontWeight:700,cursor:"pointer",textDecoration:"underline",textDecorationStyle:"dotted",
      ...
      {showFilePicker && records && (
        <div style={{position:"relative",zIndex:50}}>
          <div style={{position:"absolute",left:16,top:0,width:500,maxHeight:300,overflow:"auto",
            background:"#111",border:"1px solid #2a2a2a",borderRadius:6,boxShadow:"0 8px 32px rgba(0,0,0,0.6)"}}>
            ...
            {records.map(r => (
              <button key={r.id} onClick={()=>{onSelectRecord(r);setShowFilePicker(false);}} style={{
                ...
                padding:"8px 12px",background:r.id===record?.id?"#1a2e1a":"transparent",
                ...
                color:"#ccc",fontFamily:"'IBM Plex Mono', monospace",fontSize:11,
                 ...
                 onMouseLeave={e=>e.currentTarget.style.background=r.id===record?.id?"#1a2e1a":"transparent"}>
                <span style={{color:"#4ade80"}}>{r.filename}</span>
      ...
      <EEGControls montage={eeg.montage} setMontage={eeg.setMontage}
        ...
        rightContent={<>
          <button onClick={()=>setShowPatternTable(true)} style={controlBtn(showPatternTable)}>
            <span style={{display:"flex",alignItems:"center",gap:4}}>{I.List()} Pattern Table</span>
          </button>
          <button onClick={()=>eeg.setShowAnnotationPanel(!eeg.showAnnotationPanel)} style={controlBtn(eeg.showAnnotationPanel)}>
...
const CONN_LABELS = {
  ...
  [CONN.connected]: { text: "Connected", color: "#4ade80" },
  [CONN.impedance]: { text: "Impedance Check", color: "#8B5CF6" },
  [CONN.ready]: { text: "Ready", color: "#4ade80" },
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,minWidth:140}}>
          <div style={{width:8,height:8,borderRadius:"50%",background:connInfo.color,
            boxShadow: connectionState===CONN.scanning||connectionState===CONN.connecting ? `0 0 8px ${connInfo.color}` : "none",
          ...
          {!isConnected ? (<>
            {selectedDevice && selectedDevice.protocol === "lsl" && (
              <button onClick={onScan} disabled={connectionState===CONN.scanning} style={{
                padding:"6px 14px",background:"#111",border:"1px solid #8B5CF640",borderRadius:4,
            ...
            <button onClick={onConnect} disabled={!selectedDevice||connectionState===CONN.connecting} style={{
              ...
              border:`1px solid ${selectedDevice?"#22c55e40":"#333"}`,borderRadius:4,
              color:selectedDevice?"#4ade80":"#555",cursor:selectedDevice?"pointer":"default",
              fontSize:11,fontWeight:700,display:"flex",alignItems:"center",gap:4
            }}>{I.Zap()} CONNECT</button>
          </>) : (
            <button onClick={onDisconnect} style={{
              padding:"6px 14px",background:"#111",border:"1px solid #EF444440",borderRadius:4,
      ...
      {isConnected && selectedDevice && (
        <div style={{display:"flex",alignItems:"center",gap:16,padding:"6px 16px",borderTop:"1px solid #111",background:"#080808",fontSize:10}}>
          ...
          {selectedDevice.clinical && (<>
            <span style={{color:"#444"}}>|</span>
            <span style={{color:"#4ade80",fontWeight:700}}>FDA-CLEARED DEVICE</span>
          </>)}
        </div>
      )}
    </div>
...
function SubjectIdInput({ value, onChange }) {
  ...
  const borderColor = !hasValue ? "#222" : isValid ? "#22c55e40" : touched ? "#EF444430" : "#222";
  ...
  return (
    <div ref={wrapRef} style={{position:"relative",zIndex:40}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        ...
        {hasValue && (
          <span style={{fontSize:9,color:isValid?"#4ade80":"#555",fontFamily:"'IBM Plex Mono', monospace",minWidth:36}}>{hashSubjectId(value)}</span>
        )}
      </div>
      ...
      {focused && (
        <div style={{
          ...
          width:340,background:"#111",border:"1px solid #2a2a2a",borderRadius:6,
          ...
          <div style={{padding:"10px 12px",borderBottom:"1px solid #1a1a1a"}}>
            ...
            <div style={{fontFamily:"'IBM Plex Mono', monospace",fontSize:14,color:"#e0e0e0",marginBottom:8,letterSpacing:"0.05em"}}>
              <span style={{color:prefixDone?"#4ade80":hasValue?"#F59E0B":"#555",padding:"2px 4px",background:prefixDone?"#4ade8010":"transparent",borderRadius:2,transition:"all 0.15s"}}>
                {prefixPart || "XX"}
              </span>
              <span style={{color:hasHyphen?"#666":"#333",margin:"0 1px"}}>-</span>
              <span style={{color:numStarted?(numPart.length>=3?"#4ade80":"#F59E0B"):"#555",padding:"2px 4px",background:numPart.length>=3?"#4ade8010":"transparent",borderRadius:2,transition:"all 0.15s"}}>
                {numPart || "000"}
              </span>
            </div>
            <div style={{display:"flex",gap:16,fontSize:9,color:"#555"}}>
              <div style={{display:"flex",alignItems:"center",gap:4}}>
                <div style={{width:6,height:6,borderRadius:1,background:prefixDone?"#4ade80":"#333",transition:"background 0.15s"}}/>
                Sport / subject code (2-4 letters)
              </div>
              <div style={{display:"flex",alignItems:"center",gap:4}}>
                <div style={{width:6,height:6,borderRadius:1,background:numPart.length>=3?"#4ade80":"#333",transition:"background 0.15s"}}/>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:3}}>
              {examples.map(ex => (
                <button key={ex.prefix} onClick={()=>onChange(ex.prefix+"-"+(numPart||""))}
                  style={{
                    display:"flex",alignItems:"center",justifyContent:"space-between",
                    padding:"5px 8px",background:prefixPart===ex.prefix?"#1a2e1a":"#0a0a0a",
                    border:`1px solid ${prefixPart===ex.prefix?"#22c55e30":"#1a1a1a"}`,borderRadius:3,
                    cursor:"pointer",transition:"all 0.1s",
                  }}
                  onMouseEnter={e=>e.currentTarget.style.borderColor="#333"}
                  onMouseLeave={e=>e.currentTarget.style.borderColor=prefixPart===ex.prefix?"#22c55e30":"#1a1a1a"}>
                  <span style={{fontSize:11,fontWeight:700,color:prefixPart===ex.prefix?"#4ade80":"#aaa",fontFamily:"'IBM Plex Mono', monospace"}}>{ex.prefix}</span>
...
function PatternTable({ eegSystem, montage, channels, allChannels, hiddenChannels, toggleChannelVisibility,
  ...
  const tinySelect = { background:"#0a0a0a",border:"1px solid #222",borderRadius:2,color:"#aaa",fontSize:9,padding:"2px 3px",outline:"none",fontFamily:"'IBM Plex Mono', monospace",width:"100%" };

  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,backdropFilter:"blur(4px)"}}
      onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{
        background:"#111",border:"1px solid #2a2a2a",borderRadius:8,padding:0,
                    ...
                    <div key={ch} style={{
                      ...
                      <div style={{width:30,textAlign:"center"}}>
                        <button onClick={()=>toggleChannelVisibility(ch)} style={{
                          width:16,height:16,borderRadius:3,background:isHidden?"#1a1a1a":"#166534",
                          border:`1px solid ${isHidden?"#333":"#22c55e50"}`,cursor:"pointer",
                          display:"flex",alignItems:"center",justifyContent:"center",color:isHidden?"#555":"#4ade80",fontSize:9,
                        }}>{isHidden?" ":"✓"}</button>
                      </div>
                      ...
                      <div style={{width:56,display:"flex",justifyContent:"center"}}>
                        ...
                        }} style={{...tinySelect, color: chHpfVal !== undefined ? "#4ade80" : "#555"}}>
                          <option value="">—</option>
                      ...
                      <div style={{width:56,display:"flex",justifyContent:"center"}}>
                        ...
                        }} style={{...tinySelect, color: chLpfVal !== undefined ? "#4ade80" : "#555"}}>
                          <option value="">—</option>
                      ...
                      <div style={{width:80,display:"flex",alignItems:"center",justifyContent:"center",gap:2}}>
                        <button onClick={()=>adjustChannelSensitivity(ch,-1)} style={{
                          width:18,height:18,background:"#0a0a0a",border:"1px solid #222",borderRadius:2,
                          color:"#888",cursor:"pointer",fontSize:11,display:"flex",alignItems:"center",justifyContent:"center",
                        }}>−</button>
                        <span style={{fontSize:9,color:sens!==0?"#4ade80":"#555",fontFamily:"'IBM Plex Mono', monospace",
                      ...
                      <div style={{width:40,display:"flex",justifyContent:"center"}}>
                        <div style={{width:20,height:3,borderRadius:2,background:isEKG?"#EC4899":"#4ade80",opacity:isHidden?0.2:0.6}}/>
                      </div>
                    </div>
                  );
                })}
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 20px",borderTop:"1px solid #1a1a1a",background:"#0a0a0a"}}>
          ...
          <div style={{display:"flex",gap:6}}>
            <button onClick={()=>{setChannelHpf({});setChannelLpf({});}} style={{
              padding:"5px 12px",background:"#111",border:"1px solid #222",borderRadius:3,
              color:"#888",cursor:"pointer",fontSize:10,fontWeight:600,
            ...
            <button onClick={onClose} style={{
              padding:"5px 12px",background:"#166534",border:"1px solid #22c55e40",borderRadius:3,
              color:"#4ade80",cursor:"pointer",fontSize:10,fontWeight:700,
            }}>Done</button>
          </div>
        </div>
      </div>
...
function ImpedancePanel({ impedances, onClose, onAccept }) {
  ...
  return (
    <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:20,backdropFilter:"blur(4px)"}}>
      <div style={{background:"#111",border:"1px solid #2a2a2a",borderRadius:8,padding:24,width:560,maxHeight:"80vh",overflow:"auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))",gap:6,marginBottom:20}}>
          {impedances.map((e,i) => (
            <div key={i} style={{
              background:"#0a0a0a",border:`1px solid ${e.status==="good"?"#16653440":e.status==="fair"?"#854d0e40":"#991b1b40"}`,
              borderRadius:4,padding:"8px 10px",display:"flex",alignItems:"center",justifyContent:"space-between",
            }}>
              ...
              <span style={{fontSize:12,fontWeight:700,fontFamily:"'IBM Plex Mono', monospace",
                color:e.status==="good"?"#4ade80":e.status==="fair"?"#facc15":"#f87171"
              }}>{e.value}kΩ</span>
            </div>
          ))}
        </div>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",gap:16,fontSize:10}}>
            <span style={{color:"#4ade80"}}>● &lt;5kΩ Good</span>
            <span style={{color:"#facc15"}}>● 5-10kΩ Fair</span>
            <span style={{color:"#f87171"}}>● &gt;10kΩ Poor</span>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={onClose} style={{padding:"6px 14px",background:"#111",border:"1px solid #333",borderRadius:4,color:"#888",cursor:"pointer",fontSize:11,fontWeight:600}}>Re-check</button>
            <button onClick={onAccept} style={{
              padding:"6px 18px",background:allGood?"#166534":"#7f1d1d",
              border:`1px solid ${allGood?"#22c55e50":"#EF444450"}`,borderRadius:4,
              color:allGood?"#4ade80":"#EF4444",cursor:"pointer",fontSize:11,fontWeight:700
    ...
    <div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden",position:"relative"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,padding:"8px 16px",borderBottom:"1px solid #1a1a1a",background:"#0c0c0c",flexShrink:0}}>
        {!isRecording ? (<>
          ...
          {subjectId && (
            <div style={{padding:"6px 12px",background:"#0a0a0a",border:"1px solid #1a3a1a",borderRadius:4,fontFamily:"'IBM Plex Mono', monospace",fontSize:11,color:"#4ade80"}}>
          ...
          <button onClick={startRecording} disabled={!canRecord} style={{
            ...
            borderRadius:6,color:canRecord?"#EF4444":"#555",cursor:canRecord?"pointer":"default",
          ...
          <span style={{fontSize:11,color:"#4ade80",fontFamily:"'IBM Plex Mono', monospace"}}>{hash}</span>
          <span style={{fontSize:10,color:"#555"}}>|</span>
          ...
          <button onClick={togglePause} style={{
            padding:"6px 14px",background:"#111",border:"1px solid #F59E0B40",borderRadius:4,
          ...
          {connectionState < CONN.ready && !isRecording && (
            <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
              {connectionState === CONN.disconnected && (
                <>
                  <div style={{width:48,height:48,borderRadius:12,background:"#111",border:"1px solid #2a2a2a",
              ...
              {(connectionState === CONN.scanning || connectionState === CONN.connecting) && (
                <>
                  <div style={{width:48,height:48,borderRadius:12,background:"#111",border:"1px solid #F59E0B30",
              ...
              {connectionState === CONN.found && (
                <>
                  <div style={{width:48,height:48,borderRadius:12,background:"#111",border:"1px solid #3B82F630",
              ...
              {connectionState === CONN.connected && (
                <>
                  <div style={{width:48,height:48,borderRadius:12,background:"#111",border:"1px solid #4ade8030",
                    display:"flex",alignItems:"center",justifyContent:"center",color:"#4ade80"}}>{I.Check(20)}</div>
                  <div style={{color:"#4ade80",fontSize:14,fontWeight:600}}>Connected — running impedance check...</div>
                </>
              )}
              {connectionState === CONN.error && (
                <>
                  <div style={{width:48,height:48,borderRadius:12,background:"#111",border:"1px solid #EF444430",
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            lineHeight:1,
          }}>REACT <span style={{color:"#4ade80"}}>EEG</span></div>
          <div style={{
      ...
      <header style={{padding:"12px 24px",borderBottom:"1px solid #1a1a1a",background:"#0a0a0a",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:36,height:36,borderRadius:8,background:"linear-gradient(135deg,#166534,#052e16)",
              border:"1px solid #22c55e40",display:"flex",alignItems:"center",justifyContent:"center",color:"#4ade80"}}>
              {I.Brain()}
            </div>
            <div>
              <div style={{fontSize:16,fontWeight:800,letterSpacing:"-0.02em",color:"#e0e0e0"}}>
                REACT <span style={{color:"#4ade80"}}>EEG</span>
              </div>
              <div style={{fontSize:9,color:"#555",letterSpacing:"0.1em",fontWeight:600}}>BIOMETRIC DATA ACQUISITION & STORAGE</div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:6,color:"#4ade80",fontSize:11,fontWeight:600}}>
        ...
        <div style={{display:"flex",gap:4}}>
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
              flex:1, padding:"14px 20px", borderRadius:"8px 8px 0 0",
              background: activeTab === tab.id ? "#111" : "transparent",
              border: activeTab === tab.id ? "1px solid #2a2a2a" : "1px solid transparent",
              borderBottom: activeTab === tab.id ? "1px solid #111" : "1px solid transparent",
              color: activeTab === tab.id ? "#e0e0e0" : "#555",
              cursor: "pointer", transition: "all 0.15s",
              display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
            }}>
              <span style={{color: activeTab === tab.id ? "#4ade80" : "#444"}}>{tab.icon}</span>
